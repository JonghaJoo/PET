...
import numpy as np
import math
def preprocess_data(X,y,device,step=1):
    ...
    return X_list, y_list, len_tensor

def downsampling(X_list, y_list, max_length):
    X_downsampled, y_downsampled = [], []
    for X_seq, y_seq in zip(X_list, y_list):
        lenlen = min(len(X_seq), max_length)
        for mm in np.linspace(int(lenlen*0.6), lenlen, 6, dtype=int):
            u_length = len(X_seq)
            if u_length <= mm:
                X_downsampled.append(X_seq)
                y_downsampled.append(y_seq)
            else:
                for i in np.linspace(int(u_length*0.7), u_length, 6, dtype=int):
